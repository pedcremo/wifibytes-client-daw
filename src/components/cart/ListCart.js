			...
			const table = cartItems.items.map((item) => {
				return (
					<Table.Row key={item.id}>
						...
						<Table.Cell>
							<input value={item.quantity} onChange={(ev) => quantityItem(item, ev.target.value)} />
						</Table.Cell>
						<Table.Cell>
							<IncrementButtons item={item} quantityItem={quantityItem} function={this.props.function} />
						</Table.Cell>
						<Table.Cell>{item.price} €</Table.Cell>
						<Table.Cell>
							{item.quantity > 0 ? (item.price * item.quantity).toFixed(2) + `€` : 0 + ' €'}
						</Table.Cell>
					</Table.Row>
				);
			});

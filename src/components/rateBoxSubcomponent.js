    ...
    render() {
        ...
        const highlitedRates = this.ratesJSON.results.map((itemFiltered,index) => {
            ...
            return `      
                ...
                <h3 class="card-title text-white p-2 mr-0" style="background-color:${itemFiltered.color.hexadecimal}"> <span class="display-4">${itemFiltered.precio.toLocaleString()} ${this.T("home-euros-month")}</span> <br> ${this.T("home-vat-included")}</h3>
                <h3 class="card-text pb-4 border-bottom border-secondary"><br>${textTarifa} </h3>     
                ${subtarifas.join("")}                
                </div>
                <div class="card-footer bg-light"><a href="#" class="btn btn-primary"><h2>${this.T("view-details")}</h2></a> <a href="#" class="btn btn-secondary"><h2>${this.T("to-contract")}</h2></a></div>
                </div>
            `;
        });

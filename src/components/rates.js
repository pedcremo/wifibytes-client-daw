...
class Rates extends React.Component {
  ...
  handleFamilyPicker(event) {
    const tipo_tarifa = event.target.value;
    // 1 Movil, 2 Fijo,3 Fibra, 4 wifi, 5 TV
    if (tipo_tarifa > 0) {
      const filteredRates = this.props.tarifas.filter((item) => {
        const subs = item.subtarifas.filter((subItem) => {
  ...
  render() {
    ...
    if (Object.keys(datosEmpresa).length > 0 && ratesDescription && ratesDescription.length > 0 && rates.length > 0) {
      ...
      let boxTextsArray = [];
      for (const [key, value] of Object.entries(ratesDescription[0])) {
        if (key.startsWith('caja')) {
          const groups = key.match(/caja_([\d]*)_([\w]*)/);
